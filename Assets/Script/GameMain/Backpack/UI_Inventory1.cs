...
public class UI_Inventory1 : MonoBehaviour
{
    ...
    public void SetInventory1(Inventory1 inventory1)
    {
    ...
    private void Test1()
    {
        inventory1.GetItemDataList.Add(new ConfigItemData() { id = 1, name = "血药", iconName = "HealthPotion", explain = "aeqweq", isStackable = true, amount = 2 });
    }
    ...
    private void test1()
    {
        ...
        {
            inventory1.AddItemOnInventory(new ConfigItemData() { id = 2, name = "魔药", iconName = "ManaPotion", explain = "可以补10点魔法", isStackable = true, amount = 2 });
        }
        if (Input.GetKeyDown(KeyCode.N))//删除
        {
            inventory1.RemoveItemOnInventory(new ConfigItemData() { id = 1, name = "血药", iconName = "HealthPotion", explain = "aeqweq", isStackable = true, amount = 1 });
        }
        if (Input.GetKeyDown(KeyCode.G))//删除
        {
            inventory1.AddItemOnInventory(new ConfigItemData() { id = 1, name = "血药", iconName = "HealthPotion", explain = "aeqweq", isStackable = true, amount = 1 });
        }
    }
    /// <summary>
    /// 刷新库存物品
    ...
    public void RefreshInventoryItems()
    {
        ...
        for (int i = 0; i < inventory1.GetItemDataList.Count; i++)
        {
            ...
            ConfigItemData itemData = inventory1.GetItemDataList[i];
            //鼠标点击事件
            tfItemTemp.GetComponent<Button_UI>().ClickFunc = () =>//使用物品
            {
                //Use item
                inventory1.UseItemOnInventory(itemData);
            };

            tfItemTemp.GetComponent<Button_UI>().MouseRightClickFunc = () => //丢弃
            {
                ConfigItemData duplicateItem = new ConfigItemData()//确保itemData的引用空间不知向同一个
                {
                    id = itemData.id,
                    name = itemData.name,
                ...
                ItemWorld1.DropItem(player_Components.Player_Transform.position, duplicateItem);
                inventory1.RemoveItemOnInventory(itemData);
